...
final class SpreadsheetEngineHateosHandlerSpreadsheetDeltaInsertAfterColumn extends SpreadsheetEngineHateosHandlerSpreadsheetDeltaInsert<SpreadsheetColumnReference> {

    static SpreadsheetEngineHateosHandlerSpreadsheetDeltaInsertAfterColumn with(final SpreadsheetEngine engine,
                                                                                final SpreadsheetEngineContext context) {
        check(engine, context);
        return new SpreadsheetEngineHateosHandlerSpreadsheetDeltaInsertAfterColumn(engine, context);
    }

    private SpreadsheetEngineHateosHandlerSpreadsheetDeltaInsertAfterColumn(final SpreadsheetEngine engine,
                                                                            final SpreadsheetEngineContext context) {
    ...
    SpreadsheetDelta insert(final Range<SpreadsheetColumnReference> column,
                            final int count) {
        return this.engine.insertColumns(
                column.upperBound().value().get().addSaturated(1),
                count,
                this.context
        );
    }
